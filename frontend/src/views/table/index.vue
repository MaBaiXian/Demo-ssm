<template>
  <div class="app-container">
    ...
    <el-form ref="queryForm" :model="queryParams" size="medium" :inline="true" label-width="80px" class="form-center">
  ...
  methods: {
    fetchData() {
      ...
      getList(this.page, this.limit, this.queryParams).then(response => {
        this.list = response.data // 直接使用数组（无records层级）
        this.total = response.data.length // 或从后端其他字段获取total
...
/* Ensure the date picker and select are aligned properly */
.form-center .el-form-item {
    margin-bottom: 20px;
}
</style>
